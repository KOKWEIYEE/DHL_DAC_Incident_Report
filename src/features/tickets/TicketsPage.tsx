...
export const TicketsPage: React.FC<TicketsPageProps> = ({ tickets }) => {
  ...
  // get unique requesters for dropdown
  const uniqueRequesters = Array.from(new Set(tickets.map(t => t.requester)));
  ...
  const filteredTickets = useMemo(() => {
    return tickets.filter(ticket => {
      ...
      const matchRequester = requesterFilter === 'All' || ticket.requester === requesterFilter;
      const matchDate = dateFilter === '' || ticket.timeCreated.startsWith(dateFilter);
      return matchStatus && matchPriority && matchAssignee && matchRequester && matchDate;
    });
  }, [tickets, statusFilter, priorityFilter, assigneeFilter, requesterFilter, dateFilter]);
